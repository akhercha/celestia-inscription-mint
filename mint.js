  ...
  while (successCount < numberOfTimes) {
    try {
      ...
      const result = await client.sendTokens(
        ...
        amount,
        fee,
        base64FromBytes(Buffer.from(memo, "utf8"))
      );
      console.log(
        `${account.address}, 第 ${successCount + 1} 次操作成功: ${
...
async function main() {
  ...
  console.log(
    `资金钱包: ${walletAddress} 余额: ${
      parseFloat(balance.amount) / tokenDecimal
    }`
  );
  ...
  Promise.all(
    walletData.map((wallet) =>
      performTransaction(wallet, parseInt(process.env.MINT_TIMES))
    )
  )
    .then(() => {
      console.log("所有操作完成");
